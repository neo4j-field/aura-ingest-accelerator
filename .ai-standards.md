...
<!-- Do not edit above ## Project-Specific — run refresh-ai.sh to update -->
...
in this ecosystem. It is auto-fetched by `setup-project.sh` and `refresh-ai.sh`.
...
├── .ai-standards.md                    # Auto-generated standards + Project-Specific section
...
The AI Assistant should treat `README.md` and `ARCHITECTURE.md` as live artifacts,
...
...The AI Assistant should pause before closing the session and ask:
...
"let's stop here", "committing this"), The AI Assistant should quickly check:
...
## Working Conventions (AI Collaboration)

### Tooling
- **Preferred**: The AI Coding Agent (CLI) for active coding sessions — direct filesystem
...
### Session Continuity
- `.ai-standards.md` is the shared contract across sessions — read it at the start of
  every new coding session before touching any code
- If a parallel session produces new conventions, update `.ai-standards.md` before
...
- The AI Coding Agent commits freely on feature/* and develop branches
- Merge to main is always a deliberate human action — never ask
  The AI Coding Agent to merge or push to main
- Use --no-ff merges to preserve branch history
- Tag main after merging a completed feature or version
- The AI Coding Agent should always commit before ending a session
...
Every project contains a `.session/` directory for structured The AI Coding Agent session files.
This is the bridge between architecture/planning sessions (The AI web interface) and
implementation sessions (The AI Coding Agent).
...
### Starting a The AI Coding Agent Session

At the start of every session, before touching any code:

1. Read `.ai-standards.md` (always)
...
2. Identify any decisions that should be promoted to `specs/` or `.ai-standards.md`
3. Offer to move durable decisions to the right location
4. **Update `.ai-standards.md`:**
...
6. if the  `### Review Log` section of `.ai-standards.md` exceeds 10 entries, archive all but the 5 most recent to `/CHANGELOG.md` (append, don't overwrite), then remove the archived entries from `.ai-standards.md`

> `.ai-standards.md` must be updated in the same commit as the session file closure.
...
Session files are typically drafted in The AI web interface and dropped into `.session/` before
a The AI Coding Agent session begins. The standard handoff:

1. The AI web interface session → produces `.session/YYYY-MM-DD-topic.md`
2. File dropped into repo
3. The AI Coding Agent session: `"Read .session/2025-04-24-topic.md and proceed"`

This keeps planning and implementation cleanly separated while maintaining a full
decision audit trail in version control.# AI Module: Neo4j / Graph Standards
