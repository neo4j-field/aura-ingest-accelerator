...
2.  **Source Initialization**: A `BaseSource` (e.g., `BigQuerySource`, `GCSSource`, `DatabricksSource`) is instantiated for each job.
...
| `sources/databricks_source.py` | Streams data from Databricks Unity Catalog via SQL Warehouse. | `DatabricksSource` |
| `config.yaml` | Defines import jobs, Cypher, and source parameters. | `imports` list |

## Configuration & Secrets

- **Runtime Config**: `config.yaml`
- **Secrets**: `.env` (Neo4j credentials, GCP service account keys, Databricks PAT)
